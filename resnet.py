...
class ConvNet(nn.Module):
    def __init__(self, dl, tl, ll): #dl: iterations
        self.numClasses = 100
        super(ConvNet, self).__init__()
        self.trlayer1 = nn.Sequential(
            nn.Conv2d(3, tl[0], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(tl[0]),
            nn.ReLU(),
        ...
        for i in range(dl[0]):
            self.layer1.append(nn.Sequential(
                nn.Conv2d(tl[0], tl[0], kernel_size=3, stride=1, padding=1),
                nn.BatchNorm2d(tl[0]),
                nn.ReLU()
                ))
        self.trlayer2 = nn.Sequential(
            nn.Conv2d(tl[0], tl[1], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(tl[1]),
            nn.ReLU(),
        ...
        for i in range(dl[1]):
            self.layer2.append(nn.Sequential(
                nn.Conv2d(tl[1], tl[1], kernel_size=3, stride=1, padding=1),
                nn.BatchNorm2d(tl[1]),
                nn.ReLU()
                ))
        self.trlayer3 = nn.Sequential(
            nn.Conv2d(tl[1], tl[2], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(tl[2]),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=2, stride=2)
            )
...
#        out = self.trlayer4(out)
        ...
        return out
model = ConvNet([4, 4], [32, 32, 16], [128]).to(device)
if device == 'cuda':
